...
RobotState RobotState::from_franka(
    const franka::RobotState &franka_robot_state, std::optional<Vector7d> ddq_est, std::optional<Twist> O_dP_EE_est,
    std::optional<TwistAcceleration> O_ddP_EE_est, std::optional<double> delbow_est,
...
RobotState RobotState::from_franka(
    const franka::RobotState &franka_robot_state, const Jacobian &ee_jacobian, const Vector7d &ddq_est) {
  auto dq = toEigenD(franka_robot_state.dq);
  auto O_dp_EE_est_vec = ee_jacobian * dq;
  ...
  return RobotState::from_franka(
    franka_robot_state, ddq_est, O_dp_EE_est, O_ddp_EE_est, dq[2], ddq_est[2]);
}
