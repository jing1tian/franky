 ...
 *
 * This class provides functionality to store and manage a dynamic limit, including setting and getting the value of
 * the limit while ensuring that it cannot be written while the robot is in control.
 *
...
class DynamicsLimit {
 public:
   ...
   * Throws a runtime_error if the condition for writing to the limit is not met.
   *
   * @param value The new value to set for the limit.
   *
   * @throws std::runtime_error if the limit cannot be set due to the robot being in control.
   */
 ...
 private:
   ...
   *
   * This function checks whether the given value is within acceptable bounds for the limit. It is called before setting
   * the limit to ensure correctness.
   *
