...
from franky import Affine, JointWaypointMotion, JointWaypoint, Robot, CartesianWaypointMotion, CartesianWaypoint, \
    ReferenceType, RobotPose, ElbowState
...
if __name__ == "__main__":
    ...
    wp_motion = CartesianWaypointMotion([
        CartesianWaypoint(RobotPose(Affine([0.0, 0.0, -0.12]), ElbowState(-0.2)), ReferenceType.Relative),
        CartesianWaypoint(RobotPose(Affine([0.08, 0.0, 0.0]), ElbowState(0.0)), ReferenceType.Relative),
        CartesianWaypoint(RobotPose(Affine([0.0, 0.1, 0.0]), ElbowState(0.0)), ReferenceType.Relative),
    ])
